...
# engine vocabulary
# --------------------------------------------------------------------------
#: Real engine messages that reach level data without a constant in the port's
#: own code and without an entry in messagesList.plist (in most versions of the
#: app the editor list is the subset the tool exposes, not the whole engine).
#: Without them the audit would report a real, documented message as unknown.
CURATED_MESSAGES = (
...
def _port_message_names() -> set[str]:
    """Every ``PGE_MESSAGE_`` name the port's own source mentions.

    The engine is a port of the original's message-driven design, so the set
    of message names it handles is a faithful chunk of the engine vocabulary -
    and it is committed, so it is available on a fresh clone and inside a
    frozen build alike.
    """
    ...
    for root, _dirs, files in os.walk(pkg):
        for f in files:
            ...
            text = open(os.path.join(root, f), encoding='utf-8',
                        errors='replace').read()
            names.update(re.findall(r'PGE_MESSAGE_[A-Za-z]+', text))
    return names


def known_messages() -> set[str]:
    """Message names the engine listens for.

    The canonical source is ``ps_strings.txt``, the extraction of the original
    binary's string table (a machine that has the binary can regenerate it).
    On a fresh clone - where the binary is deliberately not committed - and
    inside a frozen build the vocabulary is instead assembled from the two
    committed sources that name the engine: the port's own source, which
    mentions every message it handles, and ``messagesList.plist``, the
    editor's parameter list - plus the handful of real engine messages that
    reach level data through neither.
    """
    ...
    out = _port_message_names()
    out.update(CURATED_MESSAGES)
    try:
        out.update(message_params())
